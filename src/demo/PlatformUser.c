...
static void testEventReport(void)
{
    PEventInfo_t event;
...
static void testStatusAlarm(void)
{
...
static void testErrorReport(void)
{
...
static void timing(uint32_t utctime)
{
    ulog("utc:%d", utctime);
    //todo timing...
}

static void ttsResultHandle(PTTSResult_t *tts)
{
    ulog("did:%s, content:%s", tts->did, tts->content);
}

static void otaNoticeHandle(PUpgradeNotice_t *notice)
{
    int i;
...
static void otaDataHandle(POTADataArgs_t *args)
{
    if(args)
    {
...
static void resourceConfig(void)
{
